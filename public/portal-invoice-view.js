...
async function load() {
  ...
  document.getElementById('mailBtn').addEventListener('click', async () => {
    ...
    if (!email) return;
    const res = await portalFetch(`/api/portal/invoices/${id}/send-email`, {
      method: 'POST',
  ...
  document.getElementById('whatsappBtn').addEventListener('click', async () => {
    const res = await portalFetch(`/api/portal/invoices/${id}/share-link`, { method: 'POST' });
    const data = await res.json();
  ...
  // "Download" from the invoice list links here with ?download=1, which
  // triggers the browser's native print-to-PDF dialog automatically once
  // the sheet has actually rendered.
